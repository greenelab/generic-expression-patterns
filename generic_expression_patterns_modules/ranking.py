...
def aggregate_stats(col_to_rank, simulated_stats_all, data_type):
    ...
    if data_type.lower() == "gsa":
        if col_to_rank == "padj" or col_to_rank == "FDR":
...
def rank_genes_or_pathways(col_to_rank, DE_summary_stats, is_template):
    ...
    if col_to_rank in ["P.Value", "adj.P.Val", "pvalue", "padj", "FDR"]:
        if is_template:
...
def format_enrichment_output(
    ...
    scores per sample
    2. ROAST and CAMERA return a matrix that is gene set x statistics.
