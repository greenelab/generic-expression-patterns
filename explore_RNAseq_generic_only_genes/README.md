...
The goal of these notebooks is to examine why these genes are uncorrelated. Specifically, we believe that these uncorrelated genes are due to differences in platform so we will test two different hypotheses that get are this.
...
**Results:**
![hypothesis_2](https://github.com/greenelab/generic-expression-patterns/blob/master/explore_uncorrelated_genes/array_expression_dist_uncorr_genes_highlight.svg)

Looks like the uncorrelated genes (red) are lowly expressed, but not more than other correlated genes (blue) captured on the array.
